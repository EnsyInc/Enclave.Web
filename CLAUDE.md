...
- **Because that rAF loop drives `mat-sidenav-content`'s margin manually, `margin-left`/`margin-right` must NOT also be in `styles.scss`'s global transition rule** — if CSS _also_ eases each JS-set value over its own duration, you get double-easing (a laggy "chasing" effect instead of 1:1 tracking). `width` itself stays in the global list (that one _is_ a pure CSS animation, on `mat-sidenav` itself, not something JS drives), but `margin-left`/`margin-right` are the rAF loop's job, not CSS's.
